...
# List of allowed origins (React frontend URL)
origins = [
    "http://localhost",
    "http://localhost:3000",
]
...
try:
    svc = pickle.load(open('../Training/svc.pkl', 'rb'))
    sym_des = pd.read_csv("../Training/symtoms_df.csv")
    precautions_df = pd.read_csv("../Training/precautions_df.csv")
    workout_df = pd.read_csv("../Training/workout_df.csv")
    description_df = pd.read_csv("../Training/description.csv")
    diets_df = pd.read_csv("../Training/diets.csv")
except FileNotFoundError:
    raise RuntimeError("One or more required training data files not found.")
...
symptoms_dict = {
    ...
    'swollen_extremities': 73, 'excessive_hunger': 74, 'extra_marital_contacts': 75,
    ...
    'scarring': 124, 'skin_peeling': 125, 'silver_like_dusting': 126,
...
def get_predicted_disease(patient_symptoms):
    ...
    try:
        predicted_disease_idx = svc.predict([input_vector])[0]
        return diseases_list.get(predicted_disease_idx, "Unknown disease")
...
def get_disease_info(disease):
    try:
        ...
        raise HTTPException(status_code=404, detail=f"Disease information not found: {disease}")
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching disease information: {str(e)}")
...
async def predict_disease(symptoms: str = Query(...)):
    try:
        ...
        patient_symptoms = [symptom.strip() for symptom in symptoms.split(',')]
        predicted_disease = get_predicted_disease(patient_symptoms)
        desc, pre, die, wrkout = get_disease_info(predicted_disease)

        return JSONResponse({
            'predicted_disease': predicted_disease,
            'description': desc,
            'precautions': pre,
            'recommended_diet': die,
            'workout': wrkout
        })
